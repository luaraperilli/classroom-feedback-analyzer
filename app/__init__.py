...
# Colunas acrescentadas depois que a tabela já existia em algum ambiente. O
# create_all() cria tabelas, mas nunca altera as que já existem — sem isto, um
...
def run_lightweight_migrations():
    """Acrescenta colunas que faltam, em SQLite ou Postgres."""
    e_sqlite = db.engine.url.get_backend_name().startswith('sqlite')

    for tabela, colunas_esperadas in _COLUNAS_NOVAS.items():
        try:
            colunas = {c['name'] for c in sa_inspect(db.engine).get_columns(tabela)}
        except Exception:
            continue
        ...
        for nome, tipo, apenas_sqlite in colunas_esperadas:
            if nome in colunas or (apenas_sqlite and not e_sqlite):
                continue

            declaracao = f'{tipo} NOT NULL DEFAULT 0' if apenas_sqlite else tipo
            db.session.execute(
                text(f'ALTER TABLE "{tabela}" ADD COLUMN {nome} {declaracao}')
            )
            db.session.commit()
            logger.info('Coluna %s adicionada à tabela %s.', nome, tabela)
